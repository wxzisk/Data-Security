...
static const uint8_t rcon[12] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C,0x8D
};
...
static void KeyExpansion192(const unsigned char *key) { 

    unsigned char temp[4]; int i, j;
...
static void AddRoundKey() {
    ...
    unsigned char tempRoundKeys[4][6], i, j, tempArray[6];
    ...
    for(i = 0; i < Nb; i++) 
    {
        ...
        tempArray[4] = roundKeys[Nk * (i + roundNumber * Nb) + 4];
        tempArray[5] = roundKeys[Nk * (i + roundNumber * Nb) + 5];
...
// static void AddRoundKey() {
//     unsigned char tempRoundKeys[4][Nb], i, j;

//     // Round keys are grabbed byte-by-byte on each round and stored in a temporal 4xNb matrix
//     for (i = 0; i < Nb; i++) {
//         for (j = 0; j < 4; j++) {
//             tempRoundKeys[j][i] = roundKeys[Nk * (i + roundNumber * Nb) + j]; 
//         }
//     }

//     // Round keys of the current round (tempRoundKeys) are XORed with state matrix
//     for (i = 0; i < 4; i++) {
//         for (j = 0; j < Nb; j++) {
//             stateMatrix[i][j] ^= tempRoundKeys[i][j];
//         }
//     }
// }
...
void aes192e(unsigned char *c, const unsigned char *p, const unsigned char *k) {
    ...
    // Iterate the process by Nr - 1 times which is 11 (12 - 1) times for 192 AES 
...
void test_aes_192(uint8_t* input)
{
    ...
    AES_set_decrypt_key(key, 192, &de_key);
